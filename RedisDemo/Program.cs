...
namespace RedisDemo
{
    class Program
    {
        ...
        {
            var client = new RedisClient("127.0.0.1", 6379);//这两个参数的值要事先在配置文件Redis.conf里有配置的,两个参数分别为host、port
            ...
            //最后一个参数为我们排序的依据
            var sortSet = client.AddItemToSortedSet("12", "百度", 400);//第1个参数为key，第2个参数为value，第3个参数为排序依据，自己指定。加入到同一个集合，所以key都相同为12
